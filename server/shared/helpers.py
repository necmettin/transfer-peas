...
from shared.models.id_only import IdOnly
...
class Helpers:
	...
	def db_select(self, tablename, conds={}):
		...
		if conds:
			...
			print(qstr, conds)
			self._db.cur.execute(qstr, conds)
		else:
			qstr += "ORDER BY Id DESC"
			self._db.cur.execute(qstr)
		rows = self.field2key(self._db.cur.fetchall())
		return rows
